...
exports.magicLoginUser = async (req, res) => {
  ...
  try {
    ...
    // const data = await db.prepare(`SELECT * FROM ${usersTable};`).all();
    const data = await db.prepare(`SELECT * FROM ${usersTable} where emailId='${userEmail}';`).all();
...
exports.setCreatorInfo = async (req, res) => {
  try {
    const { emailId, name, description, benefits, profilePic, socialUrl, nftTemplate, chatId } =
      req.body;
